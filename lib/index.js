...
function writtenNumber(n, options) {
  ...
  if (language.unitExceptions[n]) return language.unitExceptions[n] + ' ' + handleDecimales(float_part, options);
  if (alternativeBaseCardinals[n]) return alternativeBaseCardinals[n] + ' ' + handleDecimales(float_part, options);
  if (baseCardinals[n]) return baseCardinals[n] + ' ' + handleDecimales(float_part, options);
  if (n < 100)
    return handleSmallerThan100(n, language, unit,
      baseCardinals, alternativeBaseCardinals, options) + ' ' + handleDecimales(float_part, options);
  ...
  return result + ' ' + handleDecimales(float_part, options);
}


function handleDecimales(decimale_part, options) {
  let result = '';
...
console.log(writtenNumber(0, { lang: 'fr' }));
