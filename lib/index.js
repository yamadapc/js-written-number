...
function writtenNumber(n, options) {
  ...
  if (language.unitExceptions[n]) return language.unitExceptions[n] + ' ' + handleDecimales(float_part, options, language);
  if (alternativeBaseCardinals[n]) return alternativeBaseCardinals[n] + ' ' + handleDecimales(float_part, options, language);
  if (baseCardinals[n]) return baseCardinals[n] + ' ' + handleDecimales(float_part, options, language);
  if (n < 100)
    return handleSmallerThan100(n, language, unit,
      baseCardinals, alternativeBaseCardinals, options) + ' ' + handleDecimales(float_part, options, language);
  ...
  return result + ' ' + handleDecimales(float_part, options, language);
}


function handleDecimales(decimale_part, options, language) {
  let result = '';
  ...
  if (toNumber > 0) {
    result = result.concat(language.baseSeparator);
...
console.log(writtenNumber(0.5, { lang: 'en' }));
