...
function writtenNumber(n, options) {
  ...
  const int_part = Math.trunc(n);
  const float_part = Number((n - int_part)).toFixed(2);
  n = int_part;
  //
  ...
  if (language.unitExceptions[n]) return language.unitExceptions[n] + ' ' + handleDecimales(float_part, options, language.decimalSeparator);
  if (alternativeBaseCardinals[n]) return alternativeBaseCardinals[n] + ' ' + handleDecimales(float_part, options, language.decimalSeparator);
  if (baseCardinals[n]) return baseCardinals[n] + ' ' + handleDecimales(float_part, options, language.decimalSeparator);
  if (n < 100)
    return handleSmallerThan100(n, language, unit,
      baseCardinals, alternativeBaseCardinals, options) + ' ' + handleDecimales(float_part, options, language.decimalSeparator);
  ...
  return result + ' ' + handleDecimales(float_part, options, language.decimalSeparator);
}


function handleDecimales(decimale_part, options, separator) {
  let result = '';
  const floatString = decimale_part.toString();
  const toNumber = parseFloat(floatString.substring(floatString.indexOf('.') + 1, floatString.length));
  if (toNumber > 0) {
    result = result.concat(separator);
    const floatWritten = writtenNumber(toNumber, options);
    result = result.concat(floatWritten);
  }
  return result;
}
...
console.log(writtenNumber(12530.5, { lang: 'es' }));
