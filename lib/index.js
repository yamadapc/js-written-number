...
function writtenNumber(n, options) {
  ...
  if (n < 100)
    return handleSmallerThan100(n, language, baseCardinals, alternativeBaseCardinals, options);
...
function handleSmallerThan100(n, language, baseCardinals, alternativeBaseCardinals, options) {
  var dec = Math.floor(n / 10) * 10;
  var diff = n - dec;
  if (diff) {
    return (
      alternativeBaseCardinals[dec] || baseCardinals[dec] + language.baseSeparator + writtenNumber(diff, options)
    );
  }
