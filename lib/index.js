...
var languages = ["en", "es", "ar", "de", "pt", "fr", "eo", "it", "vi", "tr", "uk", "ru", "id"];
var i18n = {
...
function writtenNumber(n, options) {
  ...
  if (options.originalNumber === undefined) {
    options.originalNumber = n; // TODO do this immutably
  }
  options = util.defaults(options, writtenNumber.defaults);

  // This is used for sorting out particular cardinal exceptions / rules pertaining to specific numbers
  var originalNumber = options.originalNumber === null ? n : options.originalNumber;
  ...
  var language = typeof options.lang === "string"
    ? i18n[options.lang]
    : options.lang; // TODO we can just pass this in
  ...
  if (
    options.originalNumber &&
    language.cardinalExceptions &&
    language.cardinalExceptions[options.originalNumber]
  ) {
    return language.cardinalExceptions[options.originalNumber];
  }
  ...
  // Everything below is for numbers over 100
  // If part of the number comprises units and 10s, add that to the ret first
  if (m) {
    if (
      options.noAnd &&
  ...
  // This cycles through all the units
  for (var i = 0, len = units.length; i < len; i++) {
    // r is the quantity of this unit
    ...
    var noAnd = !((language.andException && language.andException[r]) ||
    ...
    // This makes n the remainder within this unit
    n -= r * scale[i];

    // add the number of this unit
    var sep = language.noSpaces && !unit.spaceException ? '' : ' ';

    var postSeparator = '';

    if ((unit.customSeparator && options.distanceFromRight > 0)) {
      postSeparator = language.customSeparator;
    } else if (
      unit.spaceException &&
      (
        options.distanceFromRight > 0 ||  // e.g. "ein Millionen zweitausendzwanzig" (cases where we are not on the last unit)
        n !== 0 // e.g. "ein Millionen zweitausend" (cases where there is a remainder)
      )
    ) {
      postSeparator = ' ';
    }

    ret.push(number + sep + str + postSeparator);

    options.distanceFromRight += 1;
  }
  ...
  var result = language.noSpaces ? ret.reverse().join('') : ret.reverse().join(" ");
  return result;
}
...
function handleSmallerThan100(n, language, unit, baseCardinals, alternativeBaseCardinals, options) {
  ...
  unit = n - dec;
  if (unit) {
    const writtenUnit = writtenNumber(unit, options);

    return (
      alternativeBaseCardinals[dec] || baseCardinals[dec] + language.baseSeparator + writtenUnit
    );
  }
